...
class ScraperRotation:
    def __init__(self):
        ...
        self.max_threads = (
            1  # Start with 1 thread (can be increased based on Heroku dyno)
        )
    ...
    def stop(self):
        ...
        for thread, name in threads:
            if thread and thread.is_alive():
                ...
                if thread.is_alive():
                    logger.warning(
                        f"{name.capitalize()} thread did not terminate gracefully"
                    )
    ...
    def start_scraper_thread(self):
        ...
        self.scraper_thread = threading.Thread(
            target=self.scraper_worker, name="scraper_worker", daemon=True
        )
    ...
    def start_event_processing_thread(self):
        ...
        self.event_thread = threading.Thread(
            target=self.event_processing_worker, name="event_worker", daemon=True
        )
    ...
    def start_monitor_thread(self):
        ...
        self.monitor_thread = threading.Thread(
            target=self.monitor_worker, name="monitor_worker", daemon=True
        )
    ...
    def start_log_processor_thread(self):
        ...
        self.log_processor_thread = threading.Thread(
            target=self.log_processor_worker, name="log_processor", daemon=True
        )
            ...
            try:
                ...
                if isinstance(job, str):
                    try:
                        ...
                        logger.error(
                            f"Job in event queue is a raw string and failed to parse JSON: {e} | job={job}"
                        )
                        self.queue.mark_job_failed(
                            QueueType.SCRAPER, job, error="Invalid job format"
                        )
                        continue

                if not isinstance(job, dict):
                    logger.error(
                        f"Invalid event job: expected dict but got {type(job)}"
                    )
                    self.queue.mark_job_failed(
                        QueueType.SCRAPER, job, error="Invalid job format"
                    )
                ...
                try:
                    ...
                    if success:
                        ...
                        self.queue.enqueue_job(
                            QueueType.EVENT, {"instagram_handle": instagram_handle}
                        )

                        # Mark as complete
                        self.queue.mark_job_complete(
                            QueueType.SCRAPER, instagram_handle
                        )
                        ...
                        if rate_limit_level == 2:
                            ...
                            cooldown_hours = 12
                            logger.warning(
                                f"🚨 Severe rate limits detected! Cooling down for {cooldown_hours} hours..."
                            )
                            ...
                            cooldown_hours = 6
                            logger.warning(
                                f"⚠️ Mild rate limits detected. Cooling down for {cooldown_hours} hours..."
                            )
                            self.set_rate_limit(cooldown_hours * 3600)
                            continue
                        ...
                        logger.error(
                            f"Failed to scrape {instagram_handle} after all attempts"
                        )
                        self.queue.mark_job_failed(
                            QueueType.SCRAPER,
                            instagram_handle,
                            error="Failed after retries with cookie rotation",
                        )
                    ...
                    self.queue.mark_job_failed(
                        QueueType.SCRAPER, instagram_handle, error=str(e)
                    )
                    self.status["jobs_failed"] += 1
        ...
        while self.running and not self.stop_event.is_set():
            try:
                ...
                try:
                    ...
                    self.queue.mark_job_failed(
                        QueueType.EVENT, instagram_handle, error=str(e)
                    )
        ...
        try:
            ...
            for username in clubs_to_scrape:
                if self.queue.enqueue_job(
                    QueueType.SCRAPER, {"instagram_handle": username}
                ):
                    added_count += 1

            logger.info(f"Finished populating queue with {added_count} clubs")
            self.queue.publish_notification(
                "Queue populated", {"count": added_count, "source": "auto"}
            )
    ...
    def set_rate_limit(self, duration_seconds):
        ...
        end_time = datetime.datetime.fromtimestamp(self.rate_limit_end_time).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        ...
        logger.warning(
            f"Rate limit set until {end_time} ({duration_seconds // 3600} hours)"
        )
        self.queue.publish_notification(
            "Rate limit activated",
            {"duration_hours": duration_seconds // 3600, "end_time": end_time},
        )
    ...
    def process_streams(self):
        ...
        try:
            ...
            notifications = self.queue.read_notifications(
                self.last_notification_id, count=10
            )
            ...
            status_updates = self.queue.read_status_updates(
                self.last_status_id, count=10
            )
    ...
    def handle_command(self, command, data):
        ...
        try:
            if command == "stop":
                ...
                queue_type = getattr(
                    QueueType, queue_type_str.upper(), QueueType.SCRAPER
                )
                ...
                if instagram_handle:
                    ...
                    if result:
                        logger.info(
                            f"Added club {instagram_handle} to queue with priority {priority}"
                        )
                    else:
    ...
    def check_recent_rate_limits(self, window_minutes=30) -> int:
        ...
        try:
            ...
            for line in reversed(lines):
                if (
                    "Possible rate limit detected" in line
                    or "RATE LIMIT DETECTED" in line
                ):
    ...
    def get_clubs_to_scrape(self) -> List[str]:
        ...
        try:
            ...
            response = self.db.supabase.rpc(
                "get_never_scraped_clubs", {"limit_num": self.clubs_per_session}
            ).execute()
            ...
            if len(clubs_to_scrape) < self.clubs_per_session:
                ...
                if response.data:
                    ...
                    clubs_to_scrape.extend(
                        [club["instagram_handle"] for club in oldest_scraped]
                    )
    ...
    def update_club_last_scraped(self, instagram_handle: str):
        ...
        try:
            ...
            self.db.supabase.table("clubs").update({"last_scraped": now}).eq(
                "instagram_handle", instagram_handle
            ).execute()
    ...
    def trigger_cleanup(self):
        ...
        try:
            ...
            self.queue.publish_notification(
                "Database cleanup completed", {"timestamp": time.time()}
            )
                ...
                for stream_name, messages in results:
                    for msg_id, msg_data in messages:
                        try:
                            ...
                            metrics.append({"id": msg_id.decode(), "payload": payload})
                        except Exception as e:
    ...
    def get_latest_health_metrics(self) -> Dict:
        ...
        try:
            ...
            if results:
                ...
                return {"id": msg_id.decode(), "payload": payload}
    ...
    def start_health_monitoring(self, interval_seconds: int = 60) -> bool:
        ...
        try:
            ...
            def health_monitor_worker():
                logger.info(
                    f"Health monitoring started with {interval_seconds}s interval"
                )
            ...
            thread = threading.Thread(
                target=health_monitor_worker, name="health_monitor", daemon=True
            )
            thread.start()
            ...
            self.queue.publish_notification(
                ...
                {"interval_seconds": interval_seconds, "timestamp": time.time()},
            )
        ...
        try:
            ...
            if critical_alerts:
                self.queue.publish_status(
                    "health_critical",
                    {"alerts": critical_alerts, "timestamp": time.time()},
                )
    ...
    def _scrape_with_session_rotation(self, username_list, max_cookie_attempts=2):
        ...
        for cookie_attempt in range(max_cookie_attempts):
            ...
            try:
                logger.info(
                    f"Creating new scraper session (cookie attempt {cookie_attempt + 1}/{max_cookie_attempts})"
                )
                ...
                for username in username_list:
                    ...
                    success = self._scrape_single_with_retries(
                        scraper, username, max_retries=2
                    )

                    if not success:
                        raise RateLimitDetected(
                            f"Failed to scrape {username} - likely rate limited"
                        )
                ...
                logger.error(
                    f"Session error with cookie #{cookie_attempt + 1}: {str(e)}"
                )
