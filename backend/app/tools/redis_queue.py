        ...
        try:
            ...
            health_data = {
                ...
                "disk": {
                    ...
                    "read_bytes_gb": (
                        round(disk_io.read_bytes / (1024**3), 2) if disk_io else 0
                    ),
                    "write_bytes_gb": (
                        round(disk_io.write_bytes / (1024**3), 2) if disk_io else 0
                    ),
                },
                "network": {
...
class RedisScraperQueue:
    ...
    def enqueue_job(
        self, queue_type: QueueType, job_data: Dict, priority: int = 0
    ) -> bool:
        """
        ...
        try:
            ...
            logger.info(
                f"Enqueued job to {queue_type.value} queue: {job.get('instagram_handle', 'log job')} (priority {priority})"
            )
            return True
    ...
    def get_next_job(self, queue_type: QueueType) -> Optional[Dict]:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Failed to parse job JSON from queue: {e} | job_json={job_json}"
                )
            ...
            if queue_type != QueueType.LOG and "instagram_handle" in job:
                self.redis.hset(
                    processing_key, job["instagram_handle"], json.dumps(job)
                )
                logger.info(
                    f"Started processing {queue_type.value} job: {job['instagram_handle']}. Attempt: {job['attempts']}"
                )
            else:
    ...
    def mark_job_complete(self, queue_type: QueueType, job_id: str) -> bool:
        ...
        try:
            ...
            if not job_json:
                logger.warning(
                    f"Job {job_id} not found in {queue_type.value} processing queue."
                )
                return False
            ...
            self.publish_status(
                "job_completed",
                {"queue": queue_type.value, "job_id": job_id, "timestamp": time.time()},
            )
            ...
            logger.error(
                f"Error marking {queue_type.value} job {job_id} as complete: {e}"
            )
            return False
    ...
    def mark_job_failed(
        ...
        try:
            ...
            if not job_json:
                logger.error(
                    f"Tried to mark non-existent job as failed: {job_id} in {queue_type.value} queue."
                )
                return False
            ...
            if retry and job.get("attempts", 0) < max_attempts:
                self.enqueue_job(
                    queue_type, job, priority=-10
                )  # High priority for retries
                logger.warning(
                    f"{queue_type.value} job {job_id} failed, retrying (attempt {job.get('attempts', 0)}). Error: {error}"
                )
                ...
                logger.error(
                    f"{queue_type.value} job {job_id} permanently failed after {max_attempts} attempts. Error: {error}"
                )
            ...
            logger.error(
                f"Error marking {queue_type.value} job {job_id} as failed: {e}"
            )
            return False

    def requeue_stalled_jobs(
        self, queue_type: QueueType, timeout_seconds: int = 1800
    ) -> int:
        """
        ...
        try:
            ...
            for job in stalled_jobs:
                ...
                logger.warning(
                    f"Requeued stalled {queue_type.value} job: {job_id} after timeout of {timeout_seconds} seconds."
                )

            if requeued_count > 0:
                # Publish status update
                self.publish_status(
                    "stalled_jobs_requeued",
                    {
                        "queue": queue_type.value,
        ...
        try:
            ...
            for job_id, job_json in all_processing.items():
                try:
                    ...
                    logger.error(
                        f"Error parsing job {job_id} during stalled job check: {e}"
                    )
    ...
    def flush_queue(self, queue_type: QueueType) -> int:
        ...
        try:
            ...
            logger.warning(
                f"Flushed {queue_type.value} queue. Removed {total_removed} jobs."
            )
    ...
    def publish_notification(self, message: str, data: Dict = None) -> bool:
        ...
        try:
            ...
            self.redis.xadd(
                self.notification_stream, {"payload": json.dumps(notification)}
            )
            return True
    ...
    def publish_status(self, status_type: str, data: Dict = None) -> bool:
        ...
        """
        try:
            status = {"type": status_type, "timestamp": time.time(), "data": data or {}}
                    ...
                    for msg_id, msg_data in messages:
                        try:
                            ...
                            notifications.append(
                                {"id": msg_id.decode(), "payload": payload}
                            )
                        except Exception as e:
    ...
    def get_logs(
        self, count: int = 100, level: str = None, search: str = None
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            while True:
                ...
                self.mark_job_complete(
                    QueueType.LOG, log_entry.get("id", f"log_{int(time.time())}")
                )
    ...
    def listen_to_scraper_queue(self, blocking_timeout=5):
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Started processing job: {job['instagram_handle']}. Attempt: {job['attempts']}"
                )
                return job
    ...
    def requeue_job(self, instagram_handle):
        ...
        try:
            job_json = self.redis.hget(
                self.queue_keys[QueueType.SCRAPER]["processing"], instagram_handle
            )
            if not job_json:
                logger.warning(
                    f"Job {instagram_handle} not found in processing queue. Re-adding fresh."
                )
            ...
            logger.info(
                f"Successfully requeued {instagram_handle} due to manual fallback."
            )
            return True
        except Exception as e:
