...
class SupabaseQueries:
    ...
    def ensure_category_exists(self, category_name: str) -> str:
        ...
        # Create new category
        response = (
            self.supabase.table("categories").insert({"name": category_name}).execute()
        )
    ...
    def assign_categories_to_club(self, club_id: str, categories: List[str]) -> None:
        ...
        existing_assignments = (
            self.supabase.table("clubs_categories")
            .select("category_id")
            .eq("club_id", club_id)
            .execute()
        )
        existing_category_ids = (
            [item["category_id"] for item in existing_assignments.data]
        ...
        # Categories to add
        categories_to_add = [
            id for id in new_category_ids if id not in existing_category_ids
        ]

        # Categories to remove
        categories_to_remove = [
            id for id in existing_category_ids if id not in new_category_ids
        ]

        # Add new categories
        if categories_to_add:
            assignments = [
                {"club_id": club_id, "category_id": cat_id}
        ...
        if categories_to_remove:
            for cat_id in categories_to_remove:
                self.supabase.table("clubs_categories").delete().eq(
                    "club_id", club_id
                ).eq("category_id", cat_id).execute()
            logger.info(
                f"Removed {len(categories_to_remove)} categories from club {club_id}"
            )
    ...
    def get_post_by_instagram_id(self, instagram_post_id: str) -> Optional[Dict]:
        ...
        response = (
            ...
            .select("*")
            .eq("determinant", instagram_post_id)
            .execute()
        )
    ...
    def upsert_post(self, club_id: str, post_data: Dict) -> str:
        ...
        if existing_post:
            ...
            self.supabase.table("posts").update(post_to_insert).eq(
                "id", post_id
            ).execute()
    ...
    def get_event_by_post_id(self, post_id: str) -> Optional[Dict]:
        ...
        response = (
            self.supabase.table("events").select("*").eq("post_id", post_id).execute()
        )
    ...
    def upsert_event(self, club_id: str, post_id: str, event_data: Dict) -> str:
        ...
        if existing_event:
            ...
            self.supabase.table("events").update(event_to_insert).eq(
                "id", event_id
            ).execute()
    ...
    def cleanup_unused_categories(self) -> int:
        ...
        used_categories_response = (
            self.supabase.table("clubs_categories").select("category_id").execute()
        )
        used_category_ids = (
            {item["category_id"] for item in used_categories_response.data}
            if used_categories_response.data
            else set()
        )

        # Then, get all categories
        all_categories_response = (
            self.supabase.table("categories").select("id").execute()
        )
        all_category_ids = (
            {item["id"] for item in all_categories_response.data}
            if all_categories_response.data
            else set()
        )
    ...
    def get_calendar_file(self, club_id: str) -> Optional[str]:
        ...
        """
        response = (
            self.supabase.from_("calendar_files")
            .select("ics_content")
            .eq("club_id", club_id)
            .limit(1)
            .execute()
    ...
    def save_calendar_file(self, club_id: str, ics_content: str) -> str:
        ...
        if existing_response.data and len(existing_response.data) > 0:
            ...
            response = (
                self.supabase.from_("calendar_files")
                .insert({"club_id": club_id, "ics_content": ics_content})
                .execute()
            )
    ...
    def _get_clubs_paginated_fallback(
        ...
        if category:
            ...
            count_response = (
                self.supabase.table("clubs").select("id", count="exact").execute()
            )
            total_count = (
    ...
    def search_clubs_optimized(
        ...
        try:
            ...
            response = (
                self.supabase.table("clubs")
                .select(
                    "id, name, instagram_handle, profile_image_path, description, categories(name)"
                )
    ...
    def get_club_manifest_optimized(
        self, category: Optional[str], limit: int, select_fields: str
    ) -> List[Dict]:
