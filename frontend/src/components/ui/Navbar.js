          ...
          <div className="flex items-center space-x-4">
            {!loading && user ? (
              <div className="relative" ref={dropdownRef}>
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-dark-card rounded-lg shadow-lg py-2 z-[200]">
              ...
              !loading && (
                <button
                  ...
                  className="hidden md:flex items-center px-4 py-2 text-sm font-semibold text-gray-700 dark:text-white bg-gray-100 dark:bg-dark-card rounded-full hover:bg-gray-200 dark:hover:bg-dark-gradient-start transition"
                >
        ...
        <div className={`md:hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-screen' : 'max-h-0 overflow-hidden'}`}>
          <div className="px-6 pt-6 pb-10 bg-white/50 dark:bg-dark-profile-card/70 backdrop-blur-2xl rounded-2xl mx-4 mt-2 shadow-lg space-y-8">
            ...
            {!loading && !user && (
              <button
                onClick={handleGoogleSignIn}
                className="block w-full text-center py-3 text-2xl font-semibold text-gray-800 dark:text-white bg-white/60 dark:bg-dark-profile-card/80 rounded-xl hover:bg-lavender/30 dark:hover:bg-dark-gradient-start transition"
              >
            ...
            {!loading && user && (
              <div className="pt-6 border-t dark:border-gray-600">
                <div className="flex items-center space-x-4 mb-4">
                  {user.user_metadata?.avatar_url ? (
                    <img src={user.user_metadata.avatar_url} className="h-10 w-10 rounded-full" alt="User avatar" />
                  ) : (
